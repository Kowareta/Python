...
osm = ET.Element('osm', version='0.6', generator='mp2osm_catmp' )
osm.text = '\n  '
osm.tail = '\n'
...
poitagmap = {# Warning: this particular 'gate' typecode usage is specific to Calgary Trails maps
     ...
     (0x6401,0x6400,0x6411,): {'building': 'yes'},
...
polygontagmap = {
     (0x13,): {'building': 'commercial'},
     (0x5,): { 'building': 'commercial'},
...
for line in file_mp:
    ...
    if poi or polyline or polygon:
        ...
        if line.startswith('Label'):
            ...
            tag = ET.Element('tag', k='name',v=label.strip()) # convert to title case
            tag.tail = '\n    '
            node.append(tag)
...
#f = open('id3016_'+timestr+'.osm', 'w')
f = open('id'+clientId+'_'+time.strftime("%Y%m%d%H%M")+'.osm', 'w')
#f.write(ET.tostring(osm, encoding='utf8', method='xml').decode())
f.write(ET.tostring(osm, encoding='utf8', method='xml').decode().replace("<?xml version='1.0' encoding='utf8'?>","<?xml version='1.0' encoding='utf-8'?>"))
